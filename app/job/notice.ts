...
import {ContestInfo} from "../page_object/types";
import {Job} from "../util/cron_job";

const message = (con: ContestInfo)=>
  `ばちゃこん情報
コンテスト名：${con.title}
開始時間：${con.startTime}
終了時間：${con.endTime}
...
export default class Notice implements Job {
  async run() {
    moment.locale('ja');
    ...
    const contestList
      = new Top(await Req.get(consts.PATH.top))
        .getContestUrlList()
        .filter(c => moment(c.startTime).isAfter(start) && moment(c.startTime).isBefore(end));
